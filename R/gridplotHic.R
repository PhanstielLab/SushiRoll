...
gridplotHic <- function(hic, chrom, chromstart, chromend, palette = 'reds', zrange = NULL, resolution, plottype = "square", half = NULL, raster = TRUE, addlegend = FALSE, legendlocation = NULL, height = 3.25, width=3.25, x= 0.75, y= 0.75,...){
  ...
  if (class(hic) == "data.frame"){
    ...
    if(file_ext(hic) == "hic"){
      hicregion <- extractHiC(hic, chrom = chrom, chromstart = chromstart, chromend = chromend, plottype = plottype, resolution = resolution, zrange = zrange)
    } else {
